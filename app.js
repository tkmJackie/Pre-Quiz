...
function trustedDeviceStorageKey(username) {
  const normalized = String(username || "").trim().toLowerCase();
  return `${TRUSTED_DEVICE_KEY}:${normalized}`;
}

function getTrustedDeviceToken(username) {
  const normalized = String(username || "").trim().toLowerCase();
  if (!normalized) return "";
  return localStorage.getItem(trustedDeviceStorageKey(normalized)) || "";
}

function saveTrustedDeviceToken(username, token) {
  const normalized = String(username || "").trim().toLowerCase();
  const value = String(token || "").trim();
  if (!normalized || !value) return;
  localStorage.setItem(trustedDeviceStorageKey(normalized), value);
}
...
function renderSession() {
  ...
  box.innerHTML = `
    ...
    <div class="header-actions">
      <button class="mini ghost" onclick="showProfileView()">プロフィール設定</button>
...
function settingsSidebar(active) {
  return `
    <aside class="settings-sidebar">
      <h3>プロフィール設定</h3>
      <nav>
        <button type="button" class="${active === "profile" ? "active" : ""}" onclick="showProfileView()">プロフィール</button>
        <button type="button" class="${active === "password" ? "active" : ""}" onclick="showPasswordView()">パスワード変更</button>
        <button type="button" class="${active === "twoFactor" ? "active" : ""}" onclick="showTwoFactorView()">2要素認証</button>
    ...
    <section class="simple-settings-layout">
      ...
      <section class="simple-two-factor-panel profile-settings-panel">
        ...
        <div class="simple-profile-form">
          ...
          <div class="password-row settings-password-row">
            <input id="newPassword" type="password" autocomplete="new-password" placeholder="8文字以上で入力してください">
          ...
          <p class="muted">パスワードは8文字以上で設定してください。</p>
      ...
      <section class="simple-two-factor-panel">
        ...
        ` : `
          <div id="twoFactorSetupBox" class="simple-setup-box">
            <button onclick="startTwoFactorSetup()">QRコードを表示する</button>
          </div>
        `}
      </section>
    </section>
...
async function startTwoFactorSetup() {
  try {
    ...
    box.innerHTML = `
      ...
      <button class="simple-submit-button" onclick="confirmTwoFactor()">送信</button>
    `;
  ...
  try {
    const data = await api("/api/login", {
      ...
      body: JSON.stringify({
        ...
        trustedDeviceToken: getTrustedDeviceToken($("loginUsername").value)
      })
    });
...
async function loginTwoFactor() {
  try {
    ...
    if (data.trustedDeviceToken) {
      saveTrustedDeviceToken(data.username, data.trustedDeviceToken);
    }
  ...
  return `
    <section class="card">
      ...
      <div class="filter-box">
        ...
        <button class="ghost" onclick="searchUsers()">検索</button>
      </div>
  ...
  return `
    <section class="card">
      ...
      <div class="file-row">
        ...
        <button class="danger" onclick="clearImportedExcel()">インポート済み問題を削除</button>
      </div>
      ...
      <div class="button-list">
        <button class="ghost" onclick="editQuestionSet()">選択中の問題集を編集</button>
        <button class="danger" onclick="deleteQuestionSet()">選択中の問題集を削除</button>
      </div>
    ...
    <section class="card">
      ...
      <div class="three-col">
        ...
        ${session.role === "admin" ? `
          <div>
            <label>企業</label>
            <select id="progressOrg" onchange="refreshProgressUserOptions()"></select>
          </div>
        ` : ""}
        <div>
          <label>ユーザー</label>
    ...
    <section class="card">
      ...
      <div class="three-col">
        ...
        ${session.role === "admin" ? `
          <div>
            <label>企業</label>
            <select id="answersOrg" onchange="refreshAnswersUserOptions()"></select>
          </div>
        ` : ""}
        <div>
          <label>ユーザー</label>
...
function renderUserList(users) {
  $("userList").innerHTML = tableHtml(["メールアドレス", "表示名", "企業", "ロール", "2FA", "状態", "操作"], users.map(u => [
    ...
    u.is_active ? "有効" : "削除済み",
    u.is_active ? `<button class="mini danger" onclick="deleteUser('${u.id}')">削除</button>` : ""
  ]));
}
...
function renderQuestionSetList(sets) {
  ...
  list.innerHTML = tableHtml(
    ...
    (sets || []).map(s => [
      ...
      s.question_count || 0,
      s.is_active ? "有効" : "無効",
      `<button class="mini ghost" onclick="selectQuestionSetFromList('${s.id}')">選択</button>
       <button class="mini ghost" onclick="editQuestionSet('${s.id}')">編集</button>
       <button class="mini danger" onclick="deleteQuestionSet('${s.id}')">削除</button>`
    ])
  );
}
    ...
    <div class="two-col">
      <section class="card">
        ...
        <div class="filter-box">
          ...
          <button class="ghost" onclick="searchCompanyUsers()">検索</button>
        </div>
    ...
    <section class="card quiz-screen-card">
      <div class="quiz-topbar">
        ...
        <div class="quiz-controls random-only-controls">
          ...
          <button onclick="loadQuiz()">ランダムに出題</button>
        </div>
      </div>
...
async function loadQuiz() {
  ...
  $("quizBox").innerHTML = `
    ...
    <div class="quiz-action-row">
      <button id="submitAnswerButton" onclick="submitAnswer()">回答する</button>
