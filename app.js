...
async function renderTwoFactorView() {
  ...
  const required = session?.mustSetupTwoFactor || session?.twoFactorEnabled === false;
  ...
  try {
    ...
    box.innerHTML = `
      <div class="result-box">
        ...
        ${data.otpauthUrl ? `
          <label>QRコード</label>
          <div id="twoFactorQrCode" class="two-factor-qr"></div>
          <p class="muted">
            Google AuthenticatorでQRコードを読み取ってください。読み取れない場合は、下のセットアップキーを手入力してください。<br>
            <a href="${escapeHtml(data.otpauthUrl)}">スマートフォンで認証アプリを開く</a>
          </p>
        ` : ""}
