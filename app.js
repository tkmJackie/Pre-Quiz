console.info("Zerquor LMS: export fix v20260705-15 loaded");
  ...
  root.innerHTML = `
    <section class="question-builder-grid question-builder-grid-reversed">
      <section class="question-editor-panel">
        <div class="section-title-row">
          <h4>問題入力</h4>
          <span class="pill">編集 / MathJax修正 v20260705-15</span>
        </div>
...
function renderQuestionBulkMarkdownBox(note = "Markdown形式の問題を貼り付けると、下の入力欄へ自動反映します。複数問を一括登録する場合は、各問題の末尾に ---END-QUESTION--- を入れてください。") {
  return `
    <div class="question-bulk-md-box">
      <div class="section-title-row">
        <h5>MD一括入力</h5>
        <span class="pill">自動入力 / 一括登録 / MathJax修正 v20260705-15</span>
      </div>
...
async function exportExcel() {
  ...
  try {
    ...
    const rows = await loadExportRows(setId);
    if (!rows.length) {
    ...
    showMessage(`${rows.length}問をエクスポートしました。`, "success");
  } catch (error) {
