...
function renderOrganizationList(orgs) {
  $("orgList").innerHTML = tableHtml(["企業名", "コード", "状態", "操作"], orgs.map(o => [
    ...
    escapeHtml(o.code || ""),
    o.is_active ? "有効" : "無効",
    `<button class="mini ghost" data-action="editOrganization('${o.id}')">編集</button>
     <button class="mini danger" data-action="deleteOrganization('${o.id}')">無効化</button>`
  ]));
}
...
function renderUserList(users) {
  $("userList").innerHTML = tableHtml(["メールアドレス", "表示名", "企業", "ロール", "2FA", "状態", "操作"], users.map(u => [
    ...
    u.is_active ? "有効" : "削除済み",
    u.is_active ? `<button class="mini danger" data-action="deleteUser('${u.id}')">削除</button>` : ""
  ]));
}
...
function renderQuestionSetList(sets) {
  ...
  list.innerHTML = tableHtml(
    ...
    (sets || []).map(s => [
      ...
      s.question_count || 0,
      s.is_active ? "有効" : "無効",
      `<button class="mini ghost" data-action="selectQuestionSetFromList('${s.id}')">選択</button>
       <button class="mini ghost" data-action="editQuestionSet('${s.id}')">編集</button>
       <button class="mini danger" data-action="deleteQuestionSet('${s.id}')">削除</button>`
    ])
  );
}
