...
async function api(path, options = {}) {
  ...
  if (session?.token) {
    ...
    url += `${separator}token=${encodeURIComponent(session.token)}&role=${encodeURIComponent(session.role)}`;
  }
...
async function saveWordEdit() {
  try {
    ...
    await api(`/api/admin/words/${encodeURIComponent(id)}/update`, {
      method: "POST",
      body: JSON.stringify({
